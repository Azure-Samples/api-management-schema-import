...
* Detects/resolves/inlines:
    * All `wsdl:import` directives
    * All `xsd:import` and `xsd:include` elements in XML schemas
* Merges all XML schemas with the same target namespace into a single schema, and outputs a single WSDL file
...
* Performs the following steps:
    ...
    * For each filename, generates a compliant ARM resource name:
...
- [API Management REST API](https://docs.microsoft.com/en-us/rest/api/apimanagement/current-ga/global-schema)
- [Validation policies in API Management](validation-policies.md)